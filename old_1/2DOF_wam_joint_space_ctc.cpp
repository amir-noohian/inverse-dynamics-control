...
protected:
	...
	virtual void operate() {
		...
		jt_out = 0.0 * feedFWD + gravity + kp * (jp_ref - jp_sys) + kd * (jv_ref - jv_sys);
...
int wam_main(int argc, char** argv, ProductManager& pm,	systems::Wam<DOF>& wam) {
	...
	v_type A;
	A << 0.0, inputs[0] , 0.0, inputs[1], 0.0, 0.0, 0.0;
	double f = inputs[2];
    ...
    systems::connect(refTraj.referencePTrack, wam2dofDynamics.jpInputDynamics);
	systems::connect(refTraj.referenceVTrack, wam2dofDynamics.jvInputDynamics);
    systems::connect(refTraj.referenceATrack, wam2dofDynamics.jaInputDynamics);
...
//	RT Logging stuff
	...
	systems::connect(compTorqueController.controlJtOutput, tg.template getInput<5>());
	systems::connect(wam2dofDynamics.dynamicsFeedFWD, tg.template getInput<6>());
